...
public class CSVParserAsyn implements CommandLineRunner {
    ...
    private final TmdbDataService tmdbDataService;
    private final TaskExecutor taskExecutor;
    private List<String> genreList = new ArrayList<>();
    private List<Movie> movieList = new ArrayList<>();

    @Autowired
    public CSVParserAsyn(MovieDataService movieDataService, TmdbDataService tmdbDataService, TaskExecutor taskExecutor) {
    ...
    private void getMovieDetailsAndStore(Movie movie) {
        if (movie.getTmdbId() == null || movie.getTmdbId().isEmpty())
            return;
        try {
            TmdbMovie tmdbMovie = tmdbDataService.getMovieDetails(movie.getTmdbId());
